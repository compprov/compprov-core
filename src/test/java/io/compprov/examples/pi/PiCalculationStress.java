...
public class PiCalculationStress {
    ...
    public long calculatePure(BigDecimal totalPointsBD) {
        ...
        final var pi = new BigDecimal(ACTUAL_PI);
        ...
        for (long i = 0; i < totalPointsBD.longValue(); i++) {
            ...
            // to properly estimate overhead, doing the same operations as compprov
            final var distRound = dist.setScale(0, DOWN);
            final var inCircle = one.subtract(distRound, mc);
            counter = counter.add(inCircle, mc);
        }

        // (pointsInsideBD / totalPointsBD)
        final var ratio = counter.divide(totalPointsBD, mc);
    ...
    public long calculateCompProv(BigDecimal totalPointsBD) throws IOException {
        ...
        final var roundingMc = ctx.wrapMathContext(new MathContext(0, DOWN), descriptor("0/1 math-context"));
        final var totalPoints = ctx.wrapBigDecimal(totalPointsBD, descriptor("Total points"));
        final var four = ctx.wrapBigDecimal(BigDecimal.valueOf(4), descriptor("constant 4"));
        final var two = ctx.wrapInteger(2, descriptor("constant 2 integer"));
        final var one = ctx.wrapBigDecimal(BigDecimal.ONE, descriptor("constant 1"));
        final var pi = ctx.wrapBigDecimal(new BigDecimal(ACTUAL_PI), descriptor("actual Pi"));
        ...
        final var fos = new FileOutputStream("pi.json");
    ...
    public long calculateCompProvWithFragmentation(BigDecimal totalPointsBD) throws IOException {
                ...
                new DataContext(descriptor("Pi calculation step with x,y points")));
        {
            ...
            final var roundingMc = subctx.wrapMathContext(new MathContext(0, DOWN), descriptor("0/1 math-context"));
            final var one = subctx.wrapBigDecimal(BigDecimal.ONE, descriptor("constant 1"));
            final var two = subctx.wrapInteger(2, descriptor("constant 2 integer"));
            // (x^2 + y^2) <= 1 - in circle, counter should be increased
        ...
        var counter = ctx.wrapBigDecimal(BigDecimal.ZERO, descriptor("initial counter"));
        final var piStepSubgraph = ctx.wrapSubgraph(
                new Subgraph(
                        subctx,
                        List.of(
